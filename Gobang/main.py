...
from PySide2.QtCore import QTimer,Qt,QPoint,QSize,QRect,QCoreApplication,QMetaObject,Signal,QObject
...
class Gobang(QMainWindow):
    ...
    def __init__(self):
        ...
        self.Combo_Difficulty.currentIndexChanged.connect(self.changeDifficulty)
    ...
    def setupUi(self, Form):
        ...
        self.Label_SelectFunction = QLabel(Form)
        ...
        self.Combo_Difficulty = QComboBox(Form)
    ...
    def retranslateUi(self, Form):
        ...
        self.Label_SelectFunction.setText(QCoreApplication.translate("Form", u"Select difficulty:", None))
        self.Combo_Difficulty.setItemText(0, QCoreApplication.translate("Form", u"Easy", None))
        self.Combo_Difficulty.setItemText(1, QCoreApplication.translate("Form", u"Normal", None))
        self.Combo_Difficulty.setItemText(2, QCoreApplication.translate("Form", u"Hard", None))
        self.Combo_Difficulty.setItemText(3, QCoreApplication.translate("Form", u"Master", None))

        self.Combo_Difficulty.setCurrentText(QCoreApplication.translate("Form", u"Easy", None))
        self.Combo_Difficulty.setProperty("placeholderText", "")
        self.label.setText(QCoreApplication.translate("Form", u"Evaluation value\uff1a", None))
        self.Label_Turn.setText(QCoreApplication.translate("Form", u"YOUR TURN!", None))
    # retranslateUi


    def withdraw(self):
        if self.humanAction==False:
            msgBox = QMessageBox(QMessageBox.Warning, 'Wait', 'You must wait AI', QMessageBox.Ok, self)
            msgBox.exec_()
            return
        if self.currBoard.previous is None:
            msgBox = QMessageBox(QMessageBox.Critical, 'Are you kidding?', 'The board is already empty!', QMessageBox.Ok, self)
            msgBox.exec_()
            return
    ...
    def aiWin(self):
        msgBox = QMessageBox(QMessageBox.Information, 'WHITE win', 'You LOSE!', QMessageBox.Ok, self)
        msgBox.exec_()
        self.humanAction=True
        ...
        for x in range(MAXSIZE):
            for y in range(MAXSIZE):
                if self.currBoard.status[x][y]=='X':
                    qp.drawImage((x*EACH_POS/3)+leftMargin,(y*EACH_POS/3)+topMargin,black)
                elif self.currBoard.status[x][y]=='@':
                    qp.drawImage((x*EACH_POS/3)+leftMargin,(y*EACH_POS/3)+topMargin,white)
        
        lastStepPos=self.currBoard.whitePos
        if lastStepPos != (None,None):
            if self.currBoard.status[lastStepPos[0]] [lastStepPos[1]]=='X':
                qp.drawImage((lastStepPos[0]*EACH_POS/3)+leftMargin,(lastStepPos[1]*EACH_POS/3)+topMargin,bR)
            elif self.currBoard.status[lastStepPos[0]] [lastStepPos[1]]=='@':
                qp.drawImage((lastStepPos[0]*EACH_POS/3)+leftMargin,(lastStepPos[1]*EACH_POS/3)+topMargin,wR)
        qp.end()
    ...
    def mousePressEvent(self, e) -> None:
        if e.buttons() == Qt.LeftButton and self.humanAction:
            ...
            if posX!=-1 and posY!=-1:
                # update status
                if self.currBoard.status[posX][posY]=='O':    # check if the position is empty
                    
                    # make new board
                    self.Text_Information.append("BLACK: x:"+str(posX)+" y:"+str(posY))
                    newStatus=deepcopy(self.currBoard.status)
                    newBoard=board(newStatus,self.level,self.currBoard,(posX,posY))
                    ...
                    if isBlackWin:
                        msgBox = QMessageBox(QMessageBox.Information, 'BLACK win', 'You WIN!', QMessageBox.Ok, self)
                        msgBox.exec_()
                    else:
    ...
    def aiMove(self):
        # let ui pause
        self.Label_Turn.setText(QCoreApplication.translate("Form", u"AI TURN...", None))
        self.update()
        ...
        newBoard=board(self.currBoard.status,self.level,self.currBoard,aiPos)
        ...
        # show ai move
        self.Text_Information.append("WHITE: x:"+str(aiPos[0])+" y:"+str(aiPos[1]))
